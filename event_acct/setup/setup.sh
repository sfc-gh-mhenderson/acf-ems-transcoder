...
if [ -z "${file}" ]; then
    ...
    do
        ...
        case $parameter in
            ...
            ACF_ACCOUNT_NAME)
                ...
                ;;
            EVENTS_ACCOUNT_LOCATOR)
                EVENTS_ACCOUNT_LOCATOR=$value
                printf '%s\n' "${parameter} = ${EVENTS_ACCOUNT_LOCATOR}"
                ;;  
            APP_CODE)
                APP_CODE=$value
                printf '%s\n' "${parameter} = ${APP_CODE}"
  ...
  #Native App setup
  snowsql -c $EVENTS_ACCOUNT_LOCATOR \
          -f sql/setup.sql \
          -D DIR=$DIR \
          -D ORG_NAME=$ORG_NAME \
          -D ACF_ACCOUNT_LOCATOR=$ACF_ACCOUNT_LOCATOR \
          -D ACF_ACCOUNT_NAME=$ACF_ACCOUNT_NAME \
          -D EVENTS_ACCOUNT_LOCATOR=$EVENTS_ACCOUNT_LOCATOR \
          -D APP_CODE=$APP_CODE \
