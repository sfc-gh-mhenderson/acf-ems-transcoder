...
class BasePage(Page):
    ...
    def print_page(self):
        ...
        st.title(f"Sample Data Enrichment")
        st.write(st.__version__)

    def print_sidebar(self):
        st.write("Sample Data Enrichment")
...
class home(BasePage):
    ...
    def print_page(self):
        ...
        enable_check  = session.sql(f"SELECT value FROM {app_name}.UTIL_APP.METADATA_C_V WHERE LOWER(key) = 'enabled'").to_pandas()
        setup_sql = int(pd.DataFrame(session.sql(f"SELECT COUNT(*) FROM {app_name}.SETUP.SQL").collect()).iloc[0,0])
        ...
        if setup_sql == 0 :
            ...
            if enable_check.iloc[0,0].lower()  == 'y':
