  ...
  if version_option in ["CREATE", "PATCH"]:
    ...
    with open('temp/templates/setup_script_template.txt', 'r', encoding='utf-8') as st_file:
      with open(f"setup_script.sql", 'w', encoding='utf-8') as s_file:
        s_file.write(st_file.read().replace('[[FUNCTIONS]]', func_str).replace('[[LOG_FUNCS]]',func_log_str).replace('[[PROCEDURES]]',proc_str).replace('[[LOG_PROCS]]',proc_log_str).replace('[[APP_CODE]]', app_code).replace('[[APP_MODE]]', app_mode.lower()).replace('[[LIMIT]]', limit).replace('[[LIMIT_TYPE]]', limit_type).replace('[[LIMIT_ENFORCED]]', limit_enforced))
    ...
    session.file.get(f'@P_{app_code}_SOURCE_DB_{templates_env}.ARTIFACTS.ARTIFACTS/templates/load_sidecar_template.txt', 'temp/templates')
    with open('temp/templates/load_sidecar_template.txt', 'r', encoding='utf-8') as sct_file:
      with open(f"load_sidecar.py", 'w', encoding='utf-8') as sc_file:
        sc_file.write(sct_file.read().replace('[[APP_CODE]]', app_code))
    ...
    with open(f"temp/templates/{readme_template}", 'r', encoding='utf-8') as rmt_file:
      with open(f"{readme}", 'w', encoding='utf-8') as rm_file:
        rm_file.write(rmt_file.read().replace('[[APP_CODE]]', app_code))
    ...
    session.file.put("load_sidecar.py", f"@{app_pkg_name}.VERSIONS.{version}", auto_compress=False, overwrite=True)
